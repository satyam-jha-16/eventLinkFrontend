          ...
          <TouchableOpacity disabled={ticket.entered} onPress={() => onGoToTicketPage(ticket.id)}>
            ...
            >
              <HStack>
                ...
                >
                  <HStack alignItems="flex-start" style={{
                    flex: 1,
                    flexDirection: "column",

                  }}>
                    <Text fontSize={26} bold>{ticket.event.name}</Text>
                ...
                >
                  {ticket.entered ? (<Text fontSize={16} bold color="red">{"Used"}</Text>) : (<Text fontSize={16} bold color="green">{"Available"}</Text>)}

                  {ticket.entered &&
                    <Text mt={12} fontSize={12}>{new Date(ticket.updated_at).toLocaleString()}</Text>
                  }
                </VStack>
              </HStack>
